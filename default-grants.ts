...
/**
 * Parent-child edge: parent's existing siblings default into seeing the
 * new child's record (and whatever the parent uploads about that child).
 */
...
export async function applyDefaultGrantsForRelationship(
  ...
  // spouse / adoptive_parent / guardian: no default-visibility rule defined
  // in the spec yet — intentionally a no-op until one is specified.
}
