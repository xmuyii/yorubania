 ...
 * (founding_narrative_council table) — governs both the Founding Narrative
 * and the Mission/Vision statement. Chain after requireAuth().
 */
export function requireCouncilMembership(supabase: SupabaseClient) {
  return async (req: any, res: any, next: any) => {
    if (!req.auth) return res.status(401).json({ error: "unauthenticated" });
    const { data, error } = await supabase
      .from("founding_narrative_council")
      .select("account_id")
